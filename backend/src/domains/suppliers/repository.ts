...
const SEARCHABLE_COLUMNS = ["code", "name", "contact_person", "email"] as const;
const SORTABLE_COLUMNS = ["id", "name", "code", "created_at"] as const;

export async function create(input: SupplierCreateInput, performedBy: number | null) {
  const db = getDb();
...
export async function findById(id: number, includeDeleted = false) {
  const db = getDb();
  let query = db.selectFrom("suppliers").selectAll().where("id", "=", id);
...
export async function codeExists(code: string, excludeId?: number): Promise<boolean> {
  const db = getDb();
  let query = db.selectFrom("suppliers").select("id").where("code", "=", code);
...
export async function list(params: ListParams): Promise<ListResult<Awaited<ReturnType<typeof findById>>>> {
  const db = getDb();
  let base = db.selectFrom("suppliers").where("deleted_at", "is", null);
  ...
  if (params.status) {
    base = base.where("status", "=", params.status as "active" | "inactive" | "suspended");
  }
...
export async function update(id: number, input: SupplierUpdateInput, performedBy: number | null) {
  const db = getDb();
  await db
