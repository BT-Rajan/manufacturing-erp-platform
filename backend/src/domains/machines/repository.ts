...
import type { MachineCreateInput, MachineUpdateInput } from "./schema.js";

const SEARCHABLE_COLUMNS = ["code", "name"] as const;
const SORTABLE_COLUMNS = ["id", "name", "code", "created_at"] as const;

export async function create(input: MachineCreateInput, performedBy: number | null) {
  const db = getDb();
...
export async function findById(id: number, includeDeleted = false) {
  const db = getDb();
  let query = db.selectFrom("machines").selectAll().where("id", "=", id);
...
export async function codeExists(code: string, excludeId?: number): Promise<boolean> {
  const db = getDb();
  let query = db.selectFrom("machines").select("id").where("code", "=", code);
...
export async function list(params: ListParams): Promise<ListResult<Awaited<ReturnType<typeof findById>>>> {
  const db = getDb();
  let base = db.selectFrom("machines").where("deleted_at", "is", null);
...
export async function update(id: number, input: MachineUpdateInput, performedBy: number | null) {
  const db = getDb();
  await db
