...
export async function create(input: CustomerCreateInput, performedBy: number | null) {
  const db = getDb();
...
export async function findById(id: number, includeDeleted = false) {
  const db = getDb();
  let query = db.selectFrom("customers").selectAll().where("id", "=", id);
...
export async function codeExists(code: string, excludeId?: number): Promise<boolean> {
  const db = getDb();
  let query = db.selectFrom("customers").select("id").where("code", "=", code);
...
export async function list(params: ListParams): Promise<ListResult<Awaited<ReturnType<typeof findById>>>> {
  const db = getDb();
  let base = db.selectFrom("customers").where("deleted_at", "is", null);
...
export async function update(id: number, input: CustomerUpdateInput, performedBy: number | null) {
  const db = getDb();
  await db
