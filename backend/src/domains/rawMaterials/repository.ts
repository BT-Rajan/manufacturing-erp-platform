...
import type { RawMaterialCreateInput, RawMaterialUpdateInput } from "./schema.js";

const SEARCHABLE_COLUMNS = ["code", "name"] as const;
const SORTABLE_COLUMNS = ["id", "name", "code", "created_at"] as const;

export async function create(input: RawMaterialCreateInput, performedBy: number | null) {
  const db = getDb();
  const result = await db
    .insertInto("raw_materials")
...
export async function findById(id: number, includeDeleted = false) {
  const db = getDb();
  let query = db.selectFrom("raw_materials").selectAll().where("id", "=", id);
...
export async function codeExists(code: string, excludeId?: number): Promise<boolean> {
  const db = getDb();
  let query = db.selectFrom("raw_materials").select("id").where("code", "=", code);
...
export async function list(params: ListParams): Promise<ListResult<Awaited<ReturnType<typeof findById>>>> {
  const db = getDb();
  let base = db.selectFrom("raw_materials").where("deleted_at", "is", null);
...
export async function update(id: number, input: RawMaterialUpdateInput, performedBy: number | null) {
  const db = getDb();
  await db
    .updateTable("raw_materials")
